...
def fetch_data():
    ...
    try:
        response = requests.get(settings.BASE_ENDPOINT + settings.API_ENDPOINT + settings.API_LIMIT, verify=False)
        response_time = str(round(response.elapsed.total_seconds(),2))
        log.info('Retrieved ' + settings.BASE_ENDPOINT + settings.API_ENDPOINT + settings.API_LIMIT +  ' at: ' + response_time + ' seconds')
        ...
        while response.json()['meta']['next']:
            ...
            response_time = str(round(response.elapsed.total_seconds(),2))
            log.info('Retrieved ' + settings.BASE_ENDPOINT + response.json()['meta']['next'] +  ' at: ' + response_time + ' seconds')
            yield response.json()['bioactivities']
    except Exception as e:
        log.critical('Cannot retrieve data from ' + settings.BASE_ENDPOINT + ' ' + str(e))
